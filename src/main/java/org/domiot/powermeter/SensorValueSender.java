package org.domiot.powermeter;

import com.lankheet.iot.datatypes.domotics.SensorValue;
import com.lankheet.iot.datatypes.entities.SensorType;
import com.lankheet.utils.JsonUtil;

import org.domiot.powermeter.config.MqttConfigImpl;
import org.domiot.powermeter.config.MqttTopicConfig;
import org.domiot.powermeter.config.TopicType;
 ...
 * <ul>
 * <li>It reads SensorValues from a Queue.
...
public class SensorValueSender implements Runnable {
	...
	private final MqttConfigImpl mqttConfigImpl;
	 ...
	 * @param mqttConfigImpl The mqtt configuration.
	 */
	public SensorValueSender(BlockingQueue<SensorValue> queue, MqttConfigImpl mqttConfigImpl) {
		this.queue = queue;
		this.mqttConfigImpl = mqttConfigImpl;
	}

	@Override
	public void run() {
		try {
			connectToBroker(mqttConfigImpl);
	...
	private void connectToBroker(MqttConfigImpl mqttConfigImpl) throws MqttException {
		LOG.info("Mqtt broker connection starting: {}", mqttConfigImpl);
		String userName = mqttConfigImpl.getUserName();
		String password = mqttConfigImpl.getPassword();
		mqttClient = new MqttClient(mqttConfigImpl.getUrl(), mqttConfigImpl.getClientName());
		...
		LOG.debug("Connecting to {}", mqttConfigImpl.getUrl());
		// TODO: Connect in a loop. Send notifications when broker is down for 5?
		// minutes
	...
	public void newSensorValue(SensorValue sensorValue) {
		if (!sensorValueCache.isRepeatedValue(sensorValue)) {
			...
			// Get the destination
			Optional<MqttTopicConfig> mqttTopicConfigOptional = mqttConfigImpl.getTopics().stream()
					.filter((MqttTopicConfig topicConfig) -> topicConfig.getType().getTopicName()
							.equals(topicType.getTopicName()))
					.findFirst();
