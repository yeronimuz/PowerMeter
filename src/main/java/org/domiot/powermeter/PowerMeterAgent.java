...
import org.domiot.powermeter.config.PMAgentConfig;
import org.domiot.powermeter.config.SerialPortConfig;
import org.domiot.utils.NetUtils;
 ...
 * The serial port reader is separated from the sending thread and coupled via a
 * queue.<br>
 * The capacity of the queue is QUEUE_SIZE / (VALUE_LOOP * NR_VALUES_PER_LOOP *
 * LOOPS_IN_MINUTE)<br>
 * which is 25 minutes with 7 new measurements in a 10 seconds value loop.<br>
 * So there is at least half an hour time when the MQTT connection is dropped
 * for re-establishing.
 */
public class PowerMeterAgent {
	...
	private static void showBanner() {
		try (Scanner scanner = new Scanner(
				Objects.requireNonNull(PowerMeterAgent.class.getResourceAsStream("/banner.txt")), "UTF-8")) {
	...
	public void run(String configFileName) throws IOException, InterruptedException {
		...
		MqttConfigImpl mqttConfigImpl = configuration.getMqttConfig();
		...
		Thread mqttThread = new Thread(new SensorValueSender(queue, mqttConfigImpl));
		mqttThread.start();
		...
		P1Reader serialPortReader = new P1Reader(queue, configuration.getSerialPortConfig().getP1Key(), sensorNode,
				p1Reader);
