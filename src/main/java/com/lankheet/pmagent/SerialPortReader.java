...
/**
 * <pre>
 * Wrapper around SerialPort Reader. It runs in it's own thread and puts sensorValue objects in a queue.
 * When disconnected, a retry mechanism will reconnect periodically (NR_OF_LOOPS_FOR_RETRY * SERIAL_PORT_READER_LOOP_WAIT_MS) ms
 * A serial port event occurs at random; i.e. each event captures a part of a datagram, depending on how long the wait time is.
 * </pre>
 */
public class SerialPortReader implements SerialPortEventListener, Runnable
{
   ...
   public SerialPortReader(
      BlockingQueue<SensorValue> queue, SerialPortConfig serialPortConfig, SensorNode sensorNode)
   {
      this.queue = queue;
